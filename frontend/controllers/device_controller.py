...
class DeviceController():
        ...
        if(constrains == False):
            ...
            try:
                ...
                QtWidgets.QMessageBox.information(
                    None, 'OK', 'Dispositivo registrado')

            except (CouldntCreateDevice, CouldntUpdateDevice) as create_device_exception:
                QtWidgets.QMessageBox.warning(
                    None, 'No se pudo completar la operacion', str(create_device_exception))
            except PeeweeException as critical:
                logger.exception(critical)
                QtWidgets.QMessageBox.warning(
                    None, 'No se pudo completar la operacion', 'Localmente no se pudo guardar el dispositivo')
    ...
    def load_device(self):
        ...
        try:
            ...
            self.main_ui.line_edit_nombre_dispositivo_value.setText(
                device.nombre)
            self.main_ui.label_mac_value.setText(get_device_mac())
            self.main_ui.label_estado_dispositivo_value.setText(device.estado)

            index = self.main_ui.combo_box_device_type.findText(
                device.tipo, QtCore.Qt.MatchFixedString)
            if index >= 0:
    ...
    def request_values_device(self):
        ...
        self.main_ui.combo_box_device_type.addItems(
            ['Lector de matrícula', 'Lector de huella digital'])
        execute_background(self.requesting_commom_areas)

    def requesting_commom_areas(self):

        try:
            commom_areas = get_area_comunes(
                get_current_user().idPropiedadHorizontal)
            for area in commom_areas:
                self.main_ui.combo_box_area_comun.addItem(
                    area.nombre, userData=area)

        except NotCommomAreasFound as no_commom_area_excep:
            logger.exception(no_commom_area_excep)
            QtWidgets.QMessageBox.information(
                None, 'Sorry', str(no_commom_area_excep))
